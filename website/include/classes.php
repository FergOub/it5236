...
class Application {
    ...
    public function auditlog($context, $message, $priority = 0, $userid = NULL){
      ...
      $url = "https://z7ax7vyht3.execute-api.us-east-1.amazonaws.com/default/auditlog";
      $data = array(
        'context'=>$context,
        'message'=>$message,
        'ipaddress'=>$ipaddress,
      ...
      curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-Type: application/json','Content-Length: ' . strlen($data_json), "x-api-key: CeNav9VIAS14a7KKu1tp42r9E7bxBVxM4RrD278Y")); /*  m00EfGMiGP24GmFBlneWT9aJGn4LXEXuykRPwSz7_DIFFERENT*/
    ...
    public function register($username, $password, $email, $registrationcode, &$errors) {
      ...
      $url = "https://z7ax7vyht3.execute-api.us-east-1.amazonaws.com/default/registeruser";
      $data = array(
        'userid'=>$userid,
        'username'=>$username,
    ...
    protected function sendValidationEmail($userid, $email, &$errors) {
      $emailvalidationid = bin2hex(random_bytes(16));
      $url = "https://z7ax7vyht3.execute-api.us-east-1.amazonaws.com/default/sendvalidationemail";
      $data = array(
       ...
       if($httpCode == 400) {
          ...
          if (sizeof($errors) == 0) {
            ...
            $message = "A request has been made to create an account at https://russellthackston.me for this email address. ".
                ...
                "To confirm this address, please click the following link: $pageLink?id=$emailvalidationid";
    ...
    public function newSession($userid, &$errors, $registrationcode = NULL) {
        ...
        if (sizeof($errors) == 0) {
            ...
            $url = "https://z7ax7vyht3.execute-api.us-east-1.amazonaws.com/default/newsession";
            $data = array(
            ...
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_URL, $url);
            curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-Type: application/json','Content-Length: ' . strlen($data_json), "x-api-key: CeNav9VIAS14a7KKu1tp42r9E7bxBVxM4RrD278Y")); 
            curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'POST');
            curl_setopt($ch, CURLOPT_POSTFIELDS, $data_json);
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
            $response  = curl_exec($ch);
    ...
    public function updateUserPassword($userid, $password, &$errors) {
        ...
        if(sizeof($errors) == 0) {
            ...
            $url = "https://z7ax7vyht3.execute-api.us-east-1.amazonaws.com/default/updateuserpassword";
            $data = array(
            'userid'=>$userid,
            'passwordhash'=>$passwordhash
            );
    ...
    protected function clearPasswordResetRecords($passwordresetid) {  
        $url = "https://z7ax7vyht3.execute-api.us-east-1.amazonaws.com/default/clearpasswordresetrecord";
        $data = array(
    ...
    public function login($username, $password, &$errors) {
        ...
        if (sizeof($errors) == 0) {
            ...
            if ($result === FALSE) {
                ...
                $row = $stmt->fetch();
                $MultiFactEmail = $row['email'];
                $MultiFactUserid = $row['userid'];
    ...
    public function logout() {
        ...
        // Only try to query the data into the database if there are no validation errors
        if (!empty($sessionid)) {
            $url = "https://z7ax7vyht3.execute-api.us-east-1.amazonaws.com/default/logout";
            $data = array(
            'sessionid'=>$sessionid
            );
            $data_json = json_encode($data);
    ...
    public function getThings(&$errors) {
        ...
        $url = "https://z7ax7vyht3.execute-api.us-east-1.amazonaws.com/default/getthings?registrationcode=".$registrationcode;
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $url);
        curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'GET');
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    ...
    public function getThing($thingid, &$errors) {
        ...
        if (sizeof($errors) == 0){
            $url = "https://z7ax7vyht3.execute-api.us-east-1.amazonaws.com/default/getthing?thingid=".$thingid;
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_URL, $url);
            curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'GET');
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
            ...
            } else if (isset(json_decode($response)->errorMessage)) {
    ...
    public function getComments($thingid, &$errors) {
        ...
        if (empty($thingid)) {
            ...
            $url = "https://z7ax7vyht3.execute-api.us-east-1.amazonaws.com/default/getcomments?thingid=" . $thingid;
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_URL, $url);
            curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'GET');
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    ...
    public function saveAttachment($dbh, $attachment, &$errors) {
        ...
        if (isset($attachment) && isset($attachment['name']) && !empty($attachment['name'])) {
            ...
            if (sizeof($errors) == 0) {
                ...
                $url = "https://z7ax7vyht3.execute-api.us-east-1.amazonaws.com/default/saveattachment";
                $data = array(
    ...
    public function addThing($name, $attachment, &$errors) {
        ...
        if (sizeof($errors) == 0) {
            ...
            if (sizeof($errors) == 0) {
                ...
                $url = "https://z7ax7vyht3.execute-api.us-east-1.amazonaws.com/default/addThing";
                $data = array(
                'thingid'=>$thingid,
                'thingname'=>$name,
                'thinguserid'=>$userid,
    ...
    public function addComment($text, $thingid, $attachment, &$errors) {
        ...
        if (sizeof($errors) == 0) {
            ...
            if (sizeof($errors) == 0) {
                ...
                $url = "https://z7ax7vyht3.execute-api.us-east-1.amazonaws.com/default/addcomments";
                $data = array(
                'commentid'=>$commentid,
                'commenttext'=>$text,
    ...
    public function newAttachmentType($name, $extension, &$errors) {
        ...
        if (sizeof($errors) == 0) {
            ...
            $url = "https://z7ax7vyht3.execute-api.us-east-1.amazonaws.com/default/newattachmenttype";
                $data = array(
    ...
    public function multiAuth($link, $authCode, &$errors) {
      echo "HELLO";
    }
